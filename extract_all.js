...
const cleanText = matches
  .map(t => t.replace(/\\n/g, '\n').replace(/\\"/g, '"'))
  .filter(t => t.length > 20 && !t.startsWith('http') && !t.includes('{'));

fs.writeFileSync('canva_extracted.txt', cleanText.join('\n----------------------------------------\n'));
console.log('Saved canva_extracted.txt! Total blocks:', cleanText.length);
